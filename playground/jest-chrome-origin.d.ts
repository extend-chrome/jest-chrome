...
export namespace Alarms {
   ...
   */
  export type create = jest.Mock<void, [AlarmCreateInfo]>
  /**
   * Creates an alarm. Near the time(s) specified by alarmInfo, the onAlarm event is fired. If there is another alarm with the same name (or no name if none is specified), it will be cancelled and replaced by this alarm.
   *
   * In order to reduce the load on the user's machine, Chrome limits alarms to at most once every 1 minute but may delay them an arbitrary amount more. That is, setting delayInMinutes or periodInMinutes to less than 1 will not be honored and will cause a warning. when can be set to less than 1 minute after "now" without warning but won't actually cause the alarm to fire for at least 1 minute.
   ...
   * @param alarmInfo Describes when the alarm should fire. The initial time must be specified by either when or delayInMinutes (but not both). If periodInMinutes is set, the alarm will repeat every periodInMinutes minutes after the initial event. If neither when or delayInMinutes is set for a repeating alarm, periodInMinutes is used as the default for delayInMinutes.
   */
  export type create = jest.Mock<void, [string, AlarmCreateInfo]>
   ...
   * function(boolean wasCleared) {...};
   */
  export type clear = jest.Mock<void, [(wasCleared: boolean) => void]>
  /**
   ...
   */
  export type get = jest.Mock<void, [(alarm: Alarm) => void]>
  /**
   * Retrieves details about the specified alarm.
   *
   ...
   * function( Alarm alarm) {...};
   */
  export type get = jest.Mock<void, [string, (alarm: Alarm) => void]>
...
export namespace Browser {
   ...
   */
  export type openTab = jest.Mock<void, [Options, () => void]>
   ...
   * @param options Configures how the tab should be opened.
   */
  export type openTab = jest.Mock<void, [Options]>
}
...
export namespace Bookmarks {
  ...
  export type search = jest.Mock<
    void,
    [BookmarkSearchQuery, (results: BookmarkTreeNode[]) => void]
  >
  /**
...
export namespace ContextMenus {
   ...
   * function() {...};
   */
  export type remove = jest.Mock<void, [string, (() => void)?]>
  /**
   * Removes a context menu item.
   *
   * @param menuItemId The ID of the context menu item to remove.
   ...
   * function() {...};
   */
  export type remove = jest.Mock<void, [number, (() => void)?]>
...
export namespace Downloads {
   ...
   */
  export type getFileIcon = jest.Mock<void, [number, (iconURL: string) => void]>
  /**
   * Retrieve an icon for the specified download. For new downloads, file icons are available after the onCreated event has been received. The image returned by this function while a download is in progress may be different from the image returned after the download is complete. Icon retrieval is done by querying the underlying operating system or toolkit depending on the platform. The icon that is returned will therefore depend on a number of factors including state of the download, platform, registered file types and visual theme. If a file icon cannot be determined, runtime.lastError will contain an error message.
   *
   * @param downloadId The identifier for the download.
   ...
   * function(string iconURL) {...};
   */
  export type getFileIcon = jest.Mock<
    void,
...
export namespace Extension {
   ...
   * Parameter response: The JSON response object sent by the handler of the request. If an error occurs while connecting to the extension, the callback will be called with no arguments and runtime.lastError will be set to the error message.
   */
  export type sendRequest = jest.Mock<void, [any, ((response: any) => void)?]>
  /**
...
export namespace Management {
   ...
   * function() {...};
   */
  export type uninstall = jest.Mock<void, [string, (() => void)?]>
  /**
   ...
   * function() {...};
   */
  export type uninstallSelf = jest.Mock<void, [(() => void)?]>
  /**
...
export namespace Notifications {
  ...
  export type create = jest.Mock<
    void,
    [string, NotificationOptions, ((notificationId: string) => void)?]
  >
  /**
   * Creates and displays a notification.
   *
...
export namespace Runtime {
   ...
   * @since Chrome 26.
   */
  export type connect = jest.Mock<Port, [ConnectInfo?]>
  /**
   * Attempts to connect to connect listeners within an extension/app (such as the background page), or other extensions/apps. This is useful for content scripts connecting to their extension processes, inter-app/extension communication, and web messaging. Note that this does not connect to any listeners in a content script. Extensions may connect to content scripts embedded in tabs via tabs.connect.
   *
   * @since Chrome 26.
   ...
   */
  export type connect = jest.Mock<Port, [string, ConnectInfo?]>
  /**
   ...
   */
  export type sendMessage = jest.Mock<void, [any, ((response: any) => void)?]>
  /**
   * Sends a single message to event listeners within your extension/app or a different extension/app. Similar to runtime.connect but only sends a single message, with an optional response. If sending to your extension, the runtime.onMessage event will be fired in each page, or runtime.onMessageExternal, if a different extension. Note that extensions cannot send messages to content scripts using this method. To send messages to content scripts, use tabs.sendMessage.
   *
   * @since Chrome 32.
...
export namespace Sessions {
   ...
   * Parameter devices: The list of sessions.Device objects for each synced session, sorted in order from device with most recently modified session to device with least recently modified session. tabs.Tab objects are sorted by recency in the windows.Window of the sessions.Session objects.
   */
  export type getDevices = jest.Mock<void, [(devices: Device[]) => void]>
  /**
...
export namespace System.display {
   ...
   */
  export type getInfo = jest.Mock<void, [(info: DisplayInfo[]) => void]>
  /**
   * Requests the information for all attached display devices.
   *
   * @since Chrome 59
...
export namespace Tabs {
   ...
   */
  export type getAllInWindow = jest.Mock<void, [(tab: Tab) => void]>
  /**
   * Gets details about all tabs in the specified window.
   *
   * @deprecated since Chrome 33. Please use tabs.query {windowId: windowId}.
   ...
   */
  export type getAllInWindow = jest.Mock<void, [number, (tab: Tab) => void]>
   ...
   */
  export type getSelected = jest.Mock<void, [(tab: Tab) => void]>
  /**
   * Gets the tab that is selected in the specified window.
   *
   * @deprecated since Chrome 33. Please use tabs.query {active: true}.
   ...
   * @param windowId Optional. Defaults to the current window.
   */
  export type getSelected = jest.Mock<void, [number, (tab: Tab) => void]>
  /**
   * Creates a new tab.
   *
   * @param callback Optional.
   ...
   */
  export type captureVisibleTab = jest.Mock<void, [(dataUrl: string) => void]>
  /**
   * Captures the visible area of the currently active tab in the specified window. You must have <all_urls> permission to use this method.
   *
   ...
   * Parameter dataUrl: A data URL which encodes an image of the visible area of the captured tab. May be assigned to the 'src' property of an HTML Image element for display.
   */
  export type captureVisibleTab = jest.Mock<
    void,
  ...
  export type captureVisibleTab = jest.Mock<
    void,
    [number, CaptureVisibleTabOptions, (dataUrl: string) => void]
  >
  /**
   * Reload a tab.
   *
   ...
   * @since Chrome 16.
   */
  export type reload = jest.Mock<void, [ReloadProperties, (() => void)?]>
  /**
   * Reload the selected tab of the current window.
   *
   * @since Chrome 16.
   */
  export type reload = jest.Mock<void, [(() => void)?]>
  /**
   * Duplicates a tab.
   *
   * @since Chrome 23.
  ...
  export type sendMessage = jest.Mock<
    void,
    [number, any, MessageSendOptions, ((response: any) => void)?]
  >
  /**
  ...
  export type connect = jest.Mock<runtime.Port, [number, ConnectInfo?]>
  /**
   ...
   */
  export type insertCSS = jest.Mock<void, [InjectDetails, Function?]>
  /**
   * Injects CSS into a page. For details, see the programmatic injection section of the content scripts doc.
   *
   ...
   * @param callback Optional. Called when all the CSS has been inserted.
   */
  export type insertCSS = jest.Mock<void, [number, InjectDetails, Function?]>
  /**
   ...
   */
  export type detectLanguage = jest.Mock<void, [(language: string) => void]>
  /**
   * Detects the primary language of the content in a tab.
   *
   ...
   */
  export type setZoom = jest.Mock<void, [number, (() => void)?]>
  /**
   * Zooms a specified tab.
   *
   * @since Chrome 42.
   ...
   * @param callback Optional. Called after the zoom factor has been changed.
   */
  export type setZoom = jest.Mock<void, [number, number, (() => void)?]>
  /**
   ...
   */
  export type getZoom = jest.Mock<void, [(zoomFactor: number) => void]>
  /**
   * Gets the current zoom factor of a specified tab.
   *
   * @since Chrome 42.
   ...
   * Parameter zoomFactor: The tab's current zoom factor.
   */
  export type getZoom = jest.Mock<void, [number, (zoomFactor: number) => void]>
  /**
   ...
   */
  export type setZoomSettings = jest.Mock<void, [ZoomSettings, (() => void)?]>
  /**
   * Sets the zoom settings for a specified tab, which define how zoom changes are handled. These settings are reset to defaults upon navigating the tab.
   *
   * @since Chrome 42.
   ...
   * @param callback Optional. Called after the zoom settings have been changed.
   */
  export type setZoomSettings = jest.Mock<
    void,
...
export namespace Tts {
   ...
   */
  export type speak = jest.Mock<void, [string, Function?]>
  /**
   * Speaks text using a text-to-speech engine.
   *
   * @param utterance The text to speak, either plain text or a complete, well-formed SSML document. Speech engines that do not support SSML will strip away the tags and speak the text. The maximum length of the text is 32,768 characters.
   ...
   * @param callback Optional. Called right away, before speech finishes. Check JestChrome.runtime.lastError to make sure there were no errors. Use options.onEvent to get more detailed feedback.
   */
  export type speak = jest.Mock<void, [string, SpeakOptions, Function?]>
  /**
...
export namespace Webstore {
  ...
  export type install = jest.Mock<
    void,
    [string, Function?, ((error: string, errorCode?: string) => void)?]
  >
  /**
   * @param successCallback Optional. This function is invoked when inline installation successfully completes (after the dialog is shown and the user agrees to add the item to Chrome). You may wish to use this to hide the user interface element that prompted the user to install the app or extension.
   *
...
export namespace Windows {
  ...
  export type get = jest.Mock<
    void,
    [number, GetInfo, (window: JestChrome.windows.Window) => void]
  >
  /**
  ...
  export type create = jest.Mock<
    void,
    [CreateData, ((window?: JestChrome.windows.Window) => void)?]
  >
  /**
   * Gets all windows.
   */
